...
def bezier_curve(points, num_points=100):
    ...
    for i in range(num_points):
        for j, point in enumerate(points):
            curve[i] += comb(n, j) * ((1 - t[i])**(n-j)) * (t[i]**j) * np.array(point)
    
    return curve
        
def bernstein_poly(i, n ,t):
    
    return comb(n, i) * (t ** (n - i)) * (1 - t) ** t

def getnode(f, n, x):
    #f is a n-degree poly funciton and I want to get the value of y
    y = 0.0
...
def A_star(current_map, current_pos, goal_pos):
            ...
            if x - 1 >= 0 and x - 1 < 120 and y + 1 >= 0 and y + 1 < 120:
                if x - 1 == goal_pos[0] and y + 1 == goal_pos[1]:
                    ...
                    if nodes[x][y].move == 'up_left':
                        ...
                        f1 *= Staright_reward2
                    elif nodes[x][y].move == 'down_right':
                        f1 *= PUNISH_1
                    if(nodes[x - 1][y + 1].f > f1):
                        hq.heappush(pq, (f1, [x - 1, y + 1]))
                        nodes[x - 1][y + 1].f = f1
    ...
    while nodes[x][y].p_x != -1:
        path.append([x, y])
        path_x.append(x + random.random() * 1e-2)
        path_y.append(y + random.random() * 1e-2)
        x = nodes[x][y].p_x
        y = nodes[x][y].p_y
    path_x.reverse()
    path_y.reverse()
    ...
    print(path_x)
    print(path_y)
    fitfunction = np.polyfit(path_x, path_y, len(path_x))
    path2 = []
